...
Run with:
    ...
    PYTHONPATH=. pytest tests/test_app_smoke.py -q
"""
...
def _production_python_files() -> list[str]:
    ...
    Walks the top-level packages (``app``, ``ai_agent_services``,
    ``scraping_service``, ``scoring_service``) and the
    installable ``umgl_ai`` package, skipping ``tests/``,
    ...
    (the full ML stack for ``umgl_ai``, alembic's runtime for
    ``app.db.migrations.env``) are also skipped. The smoke test
    must pass on a minimal dev install.
    """
    roots = [
        REPO_ROOT / "app",
        REPO_ROOT / "ai_agent_services",
        REPO_ROOT / "scraping_service",
        REPO_ROOT / "scoring_service",
        # umgl_ai is intentionally excluded from the import-everything
        # sweep. Its modules require torch/transformers/networkx, and
        # the package is opt-in (`make umgl-install`). A dedicated
        # test in scoring_service/tests/ covers the adapter contract
...
class ImportEveryModuleTest(unittest.TestCase):
    ...
    def test_celery_app_routes_every_declared_task(self) -> None:
        ...
        from app.tasks import crawl, extract, score, search  # noqa: F401
    ...
    def test_worker_shims_register_only_tasks_for_their_service(self) -> None:
        ...
        tasks from ``app.tasks`` (because the central celery_app
        ...
        :data:`app.service_roles.TASK_NAMES_BY_SERVICE`.
        """
        ...
        for role in (AI_AGENT_SERVICE, SCRAPING_SERVICE, SCORING_SERVICE):
            with self.subTest(role=role):
                app = create_celery_app(role)
                expected_queues = sorted({
                    f"{role.replace('_service', '')}_queue"
