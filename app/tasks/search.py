...
* :func:`generate_queries` (``app.tasks.search.generate_queries``,
  ``ai_agent_queue``) — turns a campaign into a list of web-search
  queries and dispatches :func:`execute_search` for each.

* :func:`execute_search` (``app.tasks.search.execute_search``,
  ``scraping_queue``) — runs :func:`scraping_service.crawling.search_providers.search_web`
...
@shared_task(name="app.tasks.search.generate_queries", bind=True, max_retries=2)
...
@shared_task(name="app.tasks.search.execute_search", bind=True, max_retries=3)
def execute_search(self, campaign_id: str, query: str, index: int = 0) -> dict:
    ...
    3. Fan out one :func:`app.tasks.crawl.fetch_page` task per row.
    ...
    for crawl_source_id in created_ids:
        from app.tasks.crawl import fetch_page  # local import avoids cycle at import time
