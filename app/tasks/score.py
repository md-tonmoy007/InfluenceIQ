...
* :func:`score_influencer` (``app.tasks.score.score_influencer``,
  ``scoring_queue``) — assembles a role-5 candidate from the
  influencer row + its crawl sources, runs
  :func:`scoring_service.pipeline.orchestrator.run_role5_pipeline`,
  and persists an :class:`InfluencerScore` row.

* :func:`classify_brand_safety` (``app.tasks.score.classify_brand_safety``,
  ``ai_agent_queue``) — runs
  :func:`scoring_service.analysis.brand_safety_blocklist.scan_brand_safety`
...
@shared_task(name="app.tasks.score.score_influencer", bind=True, max_retries=2)
def score_influencer(self, campaign_id: str, influencer_id: str) -> dict:
    ...
    3. Run the pipeline (deterministic unless ``UMGL_USE_*`` flags
...
@shared_task(name="app.tasks.score.classify_brand_safety", bind=True, max_retries=2)
