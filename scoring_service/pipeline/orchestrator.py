...
* :mod:`scoring_service.detection`        - 5 fake-risk detectors + classifier
* :mod:`scoring_service.analysis`         - sentiment, engagement, credibility
* :mod:`scoring_service.scoring`          - 5-layer fusion + trust formula
...
def run_role5_pipeline(candidate: dict[str, Any],
    ...
    # variants so we can record whether a umgl_ai adapter fired and
    # surface the model provenance. Graph and bot-rings v2 are inert
    # in v1 — see :mod:`scoring_service.scoring.backends.umgl_ai_adapters`.
