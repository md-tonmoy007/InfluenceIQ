...
from .backends import umgl_ai_adapters as _umgl
...
# the orchestrator can record which layers fired the umgl_ai path and
...
def semantic_signal_score_with_provenance(features: dict[str, Any]) -> SignalProvenance:
    v2_score, versions = _umgl.semantic_v2_score(features, candidate=features)
...
def behavioral_signal_score_with_provenance(scores: dict[str, Any], features: dict[str, Any]) -> SignalProvenance:
    v2_score, versions = _umgl.behavioral_v2_score(features, candidate=features)
    if v2_score is not None:
        return v2_score, True, versions
