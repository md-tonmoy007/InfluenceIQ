...
* :func:`extract_influencers` (``backend.pipeline.tasks.extract.extract_influencers``,
  ``scoring_queue``) — runs
  :func:`backend.pipeline.extraction.entities.extract_influencer_mentions`
  over a content dict, upserts ``Influencer`` and ``CrawlSource``
  rows, and dispatches :func:`backend.pipeline.tasks.score.score_influencer` for
  every newly-resolved canonical influencer.

* :func:`resolve_identity_llm` (``backend.pipeline.tasks.extract.resolve_identity_llm``,
  ``ai_agent_queue``) — LLM-assisted disambiguation of two candidate
  influencer mentions. In v1 the deterministic
  :func:`backend.pipeline.identity.resolver.resolve_candidates` already
...
def extract_influencers(self, campaign_id: str, crawl_source_id: str, content: dict) -> dict:
    ...
    3. Link the ``CrawlSource`` to each newly-attributed
       ``Influencer``.
    4. Fan out :func:`backend.pipeline.tasks.score.score_influencer` per
    ...
    for influencer_id in new_influencer_ids:
        from backend.pipeline.tasks.score import score_influencer  # local import to avoid cycle
...
def resolve_identity_llm(self, campaign_id: str, candidate_a: dict, candidate_b: dict) -> dict:
    ...
    :func:`backend.pipeline.identity.resolver.resolve_candidates`
