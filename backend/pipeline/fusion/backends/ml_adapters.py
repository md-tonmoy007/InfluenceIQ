"""Optional adapter layer for the top-level ``backend.ml`` ML package.

This module lets :mod:`backend.pipeline.fusion.components` and the
Celery adapters in :mod:`backend.pipeline.tasks.score` opt in to model-backed
...
``ML_USE_SEMANTIC_V2``     - 1 enables the registry-driven semantic engine
                                (:class:`backend.ml.semantic_v2.SemanticEngineV2`).
``ML_USE_BEHAVIORAL_V2``   - 1 enables the calibrated behavioral engine
                                (:class:`backend.ml.behavioral.BehavioralEngine`).
``ML_USE_GRAPH_V2``        - declared but inert in v1 (needs
                                :class:`backend.ml.contracts.GraphEdge` extraction).
``ML_USE_BOT_RINGS_V2``    - declared but inert in v1 (same reason).
``ML_USE_LLM_EXPLAINER``   - 1 enables the LLM natural-language explainer
                                (:class:`backend.ml.llm_explainer.LLMExplainer`)
...
ML_USE_SEMANTIC_V2: bool = _flag("ML_USE_SEMANTIC_V2")
ML_USE_BEHAVIORAL_V2: bool = _flag("ML_USE_BEHAVIORAL_V2")
ML_USE_GRAPH_V2: bool = _flag("ML_USE_GRAPH_V2")
ML_USE_BOT_RINGS_V2: bool = _flag("ML_USE_BOT_RINGS_V2")
ML_USE_LLM_EXPLAINER: bool = _flag("ML_USE_LLM_EXPLAINER")


# ---------------------------------------------------------------------------
# UUID helpers. backend.ml contracts require UUIDs for tenant_id and
...
# ---------------------------------------------------------------------------
# Lazy import cache. backend.ml pulls in torch, transformers, peft, etc.
# We never want to pay that cost unless an operator opts in. The cache
# stores one of:
#   * the backend.ml module, on success
#   * None, after a failed import attempt (we don't retry on every call)
#   * the sentinel "unknown" before the first attempt
# ---------------------------------------------------------------------------

_UNKNOWN: Any = object()
_ML_CACHE: dict[str, Any] = {"module": _UNKNOWN}


def reset_import_cache() -> None:
    """Reset the import cache. Tests call this between cases."""
    _ML_CACHE["module"] = _UNKNOWN


def _try_import_ml() -> Any | None:
    """Return the backend.ml module or None. Result is cached for the process lifetime."""
    cached = _ML_CACHE.get("module", _UNKNOWN)
    if cached is not _UNKNOWN:
        return cached
    try:
        import backend.ml as ml_module  # type: ignore[import-not-found]
    except Exception:
        _ML_CACHE["module"] = None
        return None
    _ML_CACHE["module"] = ml_module
    return ml_module
...
def semantic_v2_score(features: dict[str, Any], *,
    ...
    Returns ``(None, {})`` when the flag is off, backend.ml is not
    importable, no text is available, or the engine raised. Never
    raises.
    """
    if not ML_USE_SEMANTIC_V2:
        return None, {}
    ml_pkg = _try_import_ml()
    if ml_pkg is None:
        return None, {}
...
def behavioral_v2_score(features: dict[str, Any], *,
    ...
    onto the :class:`backend.ml.contracts.BehaviorFeatures` Pydantic model.
    Returns ``(None, {})`` when the flag is off, the import fails, or the
    engine raised. Never raises.
    """
    if not ML_USE_BEHAVIORAL_V2:
        return None, {}
    ml_pkg = _try_import_ml()
    if ml_pkg is None:
        return None, {}
    ...
    try:
        ...
        from backend.ml.contracts import BehaviorFeatures
