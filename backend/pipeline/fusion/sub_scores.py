...
from backend.pipeline.analysis.credibility import calculate_credibility, confidence_for_source_count
...
from backend.pipeline.fusion.versioning import MODEL_VERSION, computed_at
...
def build_role4_scores(candidate: dict[str, Any], campaign: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    trust = calculate_role4_trust(sub_scores, data_source_count=source_count, severe_brand_safety=severe)
    sub_scores.update({"role4_trust_score": trust.role4_trust_score, "role4_fake_risk_score": round(fusion.score * 100, 2)})
    ...
    score_explanations = {
        ...
        "role4_fake_risk_score": {"reasons": ["Five-layer fake risk uses renormalized available components"],
                                  "evidence": risk_score["components"]},
        "role4_trust_score": {"reasons": list(trust.caps), "evidence": {"positive_trust_score": trust.positive_trust_score, "fake_risk_penalty": trust.fake_risk_penalty}},
    }
...
def build_influencer_output(candidate: dict[str, Any], campaign: dict[str, Any] | None = None) -> dict[str, Any]:
    result = build_role4_scores(candidate, campaign)
    compact = {"relevance": result["sub_scores"]["relevance_score"], "credibility": result["sub_scores"]["credibility_score"],
        ...
        "overall_fake_risk": result["sub_scores"]["overall_fake_risk_score"], "role4_trust_score": result["sub_scores"]["role4_trust_score"]}
...
def build_sub_scores(candidate: dict[str, Any], campaign: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {"sub_scores": {"relevance": full["relevance_score"], "credibility": full["credibility_score"],
            ...
            "analysis": {**result["analysis"], "role4": result}}
