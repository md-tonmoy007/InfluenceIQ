...
class StrengthsRisksTests(unittest.TestCase):
    ...
    def test_strong_positive_sentiment_produces_strength(self) -> None:
        external = {"_coverage": {}}
        strengths, risks = _derive_strengths_risks(sentiment=85.0, fake_risk=10.0, external=external)
    ...
    def test_high_fake_risk_produces_risk(self) -> None:
        external = {"_coverage": {}}
        strengths, risks = _derive_strengths_risks(sentiment=50.0, fake_risk=65.0, external=external)
    ...
    def test_elevated_fake_risk_produces_risk(self) -> None:
        external = {"_coverage": {}}
        strengths, risks = _derive_strengths_risks(sentiment=55.0, fake_risk=45.0, external=external)
        self.assertIn("Elevated fake engagement risk", risks)

    def test_trends_ok_adds_strength(self) -> None:
        external = {"_coverage": {"google_trends": "ok"}}
        strengths, risks = _derive_strengths_risks(sentiment=50.0, fake_risk=10.0, external=external)
...
class RecommendationTests(unittest.TestCase):
    ...
    def test_no_comments_returns_insufficient_data(self) -> None:
        result = _build_recommendation(sentiment=90.0, fake_risk=0.0, total_comments=0)
        self.assertIn("Insufficient comment data", result)

    def test_high_fake_risk_returns_caution(self) -> None:
        result = _build_recommendation(sentiment=90.0, fake_risk=75.0, total_comments=100)
        self.assertIn("Proceed with caution", result)

    def test_strong_sentiment_returns_partnership(self) -> None:
        result = _build_recommendation(sentiment=80.0, fake_risk=10.0, total_comments=200)
        self.assertIn("Strong audience sentiment supports partnership", result)

    def test_mixed_sentiment_returns_review(self) -> None:
        result = _build_recommendation(sentiment=60.0, fake_risk=10.0, total_comments=200)
        self.assertIn("Mixed audience sentiment", result)

    def test_weak_sentiment_returns_consider_others(self) -> None:
        result = _build_recommendation(sentiment=30.0, fake_risk=10.0, total_comments=200)
...
class GradeTests(unittest.TestCase):
    ...
    def test_perfect_grade(self) -> None:
        self.assertEqual(_grade_from_scores(sentiment=95.0, fake_risk=0.0), "A")

    def test_good_grade(self) -> None:
        self.assertEqual(_grade_from_scores(sentiment=75.0, fake_risk=5.0), "B")

    def test_average_grade(self) -> None:
        self.assertEqual(_grade_from_scores(sentiment=65.0, fake_risk=10.0), "C")

    def test_poor_grade(self) -> None:
        self.assertEqual(_grade_from_scores(sentiment=50.0, fake_risk=20.0), "D")

    def test_fail_grade(self) -> None:
        self.assertEqual(_grade_from_scores(sentiment=20.0, fake_risk=60.0), "F")

    def test_high_fake_risk_drags_grade_down(self) -> None:
        grade_clean = _grade_from_scores(sentiment=75.0, fake_risk=0.0)
        grade_risky = _grade_from_scores(sentiment=75.0, fake_risk=70.0)
...
class ReportPayloadSchemaTests(unittest.TestCase):
    ...
    def _build_minimal_payload(self) -> dict:
        ...
        strengths, risks = _derive_strengths_risks(sentiment=70.0, fake_risk=15.0, external=external)
        recommendation = _build_recommendation(sentiment=70.0, fake_risk=15.0, total_comments=100)
...
class NoSocialURLsTests(unittest.TestCase):
    ...
    def test_no_comments_prefix_yields_insufficient_recommendation(self) -> None:
        self.assertIn("Insufficient comment data",
                       _build_recommendation(sentiment=80.0, fake_risk=0.0, total_comments=0))
