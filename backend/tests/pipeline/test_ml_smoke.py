"""Smoke tests for the optional ``backend.ml`` integration.

These tests do not require ``backend.ml`` to be installed. They verify the
...
def test_try_import_returns_none_when_umgl_ai_missing(monkeypatch):
    """Without ``backend.ml`` on ``sys.path`` the helper returns ``None`` and never raises."""
    from backend.pipeline.fusion.backends import ml_adapters as adp

    monkeypatch.delitem(sys.modules, "backend.ml", raising=False)
    monkeypatch.setitem(sys.modules, "backend.ml", None)
...
def test_semantic_v2_returns_none_when_flag_off():
    ...
    adp.reset_import_cache()
    os.environ["ML_USE_SEMANTIC_V2"] = "0"
...
def test_behavioral_v2_returns_none_when_flag_off():
    ...
    adp.reset_import_cache()
    os.environ["ML_USE_BEHAVIORAL_V2"] = "0"
...
def test_explain_via_llm_returns_empty_when_disabled():
    ...
    os.environ["ML_USE_LLM_EXPLAINER"] = "0"
