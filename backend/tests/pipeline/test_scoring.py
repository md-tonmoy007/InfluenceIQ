...
from backend.pipeline.fusion.sub_scores import build_influencer_output, build_role4_scores
...
def test_trust_formula_and_caps() -> None:
    ...
    assert calculate_role4_trust(base, data_source_count=6).role4_trust_score == 100
    assert calculate_role4_trust({**base, "overall_fake_risk_score": 90}, data_source_count=6).role4_trust_score <= 45
    assert calculate_role4_trust(base, data_source_count=1).role4_trust_score == pytest.approx(23.33, abs=0.01)
    assert calculate_role4_trust(base, data_source_count=6, severe_brand_safety=True).role4_trust_score == 40
...
def test_full_output_contract_and_reasons() -> None:
    ...
    scores = build_role4_scores(candidate)
    output = build_influencer_output(candidate)
    assert scores["sub_scores"]["role4_trust_score"] >= 0
    assert output["risk_score"]["model_version"] in (MODEL_VERSION, MODEL_VERSION_V2)
