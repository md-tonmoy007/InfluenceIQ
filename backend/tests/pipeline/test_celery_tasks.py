...
* :func:`backend.pipeline.tasks.search.generate_queries` produces a non-empty
  query list and dispatches one :func:`execute_search` task per
  query (verified via the ``fetch_url`` mock counting how many
  pages were fetched).
* :func:`backend.pipeline.tasks.crawl.fetch_page` and
  :func:`extract_content` chain through correctly.
* :func:`backend.pipeline.tasks.extract.extract_influencers` finds at least
  one influencer mention in a real-looking HTML payload.
* :func:`backend.pipeline.tasks.score.score_influencer` returns a score in the
...
from backend.core.celery.app import celery_app  # noqa: E402
...
def _patched_db_session():
    ...
    lifecycle.
    """
    import backend.core.cache.event_log as event_log_mod
    import backend.core.cache.pipeline_state as state_mod
    import backend.core.cache.redis_client as redis_client_mod
    import backend.pipeline.tasks._common as common
    import backend.pipeline.tasks.crawl as crawl_mod
    import backend.pipeline.tasks.extract as extract_mod
    import backend.pipeline.tasks.score as score_mod
    import backend.pipeline.tasks.search as search_mod
...
class PipelineE2ETest(unittest.TestCase):
    ...
    def test_full_chain_calls_all_stages(self) -> None:
        ...
        with _patched_db_session(), \
             patch("backend.pipeline.tasks.search.execute_search.delay", side_effect=_fake_execute_search), \
        ...
        with _patched_db_session(), \
             patch("backend.pipeline.tasks.crawl.extract_content.delay") as extract_delay, \
             patch("backend.pipeline.tasks.crawl.fetch_url", side_effect=_stub_fetch_url):
